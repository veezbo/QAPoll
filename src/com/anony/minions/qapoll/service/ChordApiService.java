...
package com.anony.minions.qapoll.service;
...
public class ChordApiService extends Service {
	...
	public interface IChordServiceListener {
		...
		void onFileWillReceive(String node, String channel, String fileName,
				String exchangeId);

		void onFileProgress(boolean bSend, String node, String channel,
		...
		void onFileCompleted(int reason, String node, String channel,
	...
	public int start(int interfaceType) {
		...
		return mChord.start(interfaceType, new IChordManagerListener() {
			...
			public void onStarted(String name, int reason) {
				...
				IChordChannel channel = mChord.joinChannel(
						ChordManager.PUBLIC_CHANNEL, mChannelListener);
	...
	private IChordChannelListener mChannelListener = new IChordChannelListener() {
		...
		public void onDataReceived(String fromNode, String fromChannel,
			...
			byte[] buf = payload[0];
			if (null != mListener)
				mListener.onReceiveMessage(fromNode, fromChannel, new String(
						buf));
		}
		...
		public void onFileWillReceive(String fromNode, String fromChannel,
			...
			Log.d(TAG, TAGClass + "[originalName : " + fileName + " from : "
					+ fromNode + " exchangeId : " + exchangeId + " fileSize : "
					+ fileSize + "]");
			...
			if (availableMemory < fileSize) {
				...
				if (null != mListener)
					mListener.onFileCompleted(IChordServiceListener.FAILED,
		...
		public void onFileChunkReceived(String fromNode, String fromChannel,
			...
			if (null != mListener) {
				...
				mListener.onFileProgress(false, fromNode, fromChannel,
						progress, exchangeId);
			}
		}
		...
		public void onFileReceived(String fromNode, String fromChannel,
			...
			if (null != mListener) {
				mListener.onFileCompleted(IChordServiceListener.RECEIVED,
		...
		public void onFileChunkSent(String toNode, String toChannel,
			...
			if (null != mListener) {
				int progress = (int) (offset * 100 / fileSize);
				mListener.onFileProgress(true, toNode, toChannel, progress,
						exchangeId);
			}
		}
		...
		public void onFileSent(String toNode, String toChannel,
			...
			if (null != mListener) {
				mListener.onFileCompleted(IChordServiceListener.SENT, toNode,
		...
		public void onFileFailed(String node, String channel, String fileName,
			...
			case ERROR_FILE_REJECTED: {
				Log.e(TAG, TAGClass + "onFileFailed() - REJECTED Node : "
						+ node + ", fromChannel : " + channel);
				if (null != mListener) {
					mListener.onFileCompleted(IChordServiceListener.REJECTED,
			...
			case ERROR_FILE_CANCELED: {
				Log.e(TAG, TAGClass + "onFileFailed() - CANCELED Node : "
						+ node + ", fromChannel : " + channel);
				if (null != mListener) {
					mListener.onFileCompleted(IChordServiceListener.CANCELLED,
							node, channel, exchangeId, fileName);
				}
				break;
			}
			...
			default:
				Log.e(TAG, TAGClass + "onFileFailed() - " + reason + " : "
						+ node + ", fromChannel : " + channel);
				if (null != mListener) {
					mListener.onFileCompleted(IChordServiceListener.FAILED,
							node, channel, exchangeId, fileName);
	...
	public String sendFile(String toChannel, String strFilePath, String toNode) {
		 ...
		 */
		return channel.sendFile(toNode, MESSAGE_TYPE_FILE_NOTIFICATION,
				strFilePath, SHARE_FILE_TIMEOUT_MILISECONDS);
	}
	...
	public List<String> getJoinedNodeList(String channelName) {
		...
		if (null == channel) {
			Log.e(TAG, TAGClass
					+ "getJoinedNodeList() : invalid channel instance-"
					+ channelName);
			return null;
		}
	...
	public boolean sendData(String toChannel, byte[] buf, String nodeName) {
		...
		Log.v(TAG, TAGClass + "sendData : " + new String(buf) + ", des : "
				+ nodeName);
		 ...
		 * is returned
		 */
		if (false == channel.sendData(nodeName, CHORD_APITEST_MESSAGE_TYPE,
				payload)) {
	...
	public String getNodeIpAddress(String channelName, String nodeName) {
		Log.d(TAG, TAGClass + "getNodeIpAddress() channelName : " + channelName
				+ ", nodeName : " + nodeName);
		// Request the channel interface for the specific channel name.
		IChordChannel channel = mChord.getJoinedChannel(channelName);
		if (null == channel) {
	...
	public IChordChannel joinChannel(String channelName) {
		 ...
		 * null otherwise.
		 */
		IChordChannel channelInst = mChord.joinChannel(mPrivateChannelName,
				mChannelListener);
	...
	public void setListener(IChordServiceListener listener) {
		mListener = listener;
	}
